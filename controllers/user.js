...
exports.signupRegularUser = asyncHandler(async (req, res, next) => {
	...
	let responseObject = {
		...
		message: "Welcome to Kemon Achen! Let's take a step towards healing together <3",
	};
...
exports.signupProfessionalUser = asyncHandler(async (req, res, next) => {
	const {role, email, password, verified, license, licenseIssued, specializations} = req.body;
...
exports.joinCommunity = asyncHandler(async (req, res, next) => {
	...
	if (community.length != 0) {
		return next(new Error(`User with id: ${req.user._id} has already joined the community`, 400));
	}
	...
	if (!updatedCommunity) {
		return next(
			new ErrorResponse(`Failed to update Community with id: ${id} with User: ${req.user._id}`)
		);
	}

	res.status(200).json({message: "You are added to the community successfully"});
});
