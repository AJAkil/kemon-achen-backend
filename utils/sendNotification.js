...
exports.sendNotifications = async () => {
    ...
    if (
      ...
      for (const pushtoken of iterator) {
        tokenMappingToUser.push({
          userId: user._id,
          pushToken: pushtoken,
          adviceIds: user.testInfo[0].advice,
        });
      }
    }
  });
  ...
  let messages = [];
  for (let user of tokenMappingToUser) {
    ...
    messages.push({
      to: pushToken,
      sound: 'default',
      body: 'Have you followed our advice?',
      data: { adviceIds: user.adviceIds, _displayInForeground: true },
    });
  }
